...
exports.addReview=async (req,res)=>{
    ...
    const movie=await Movie.findOne({_id:movieId,status:"public"});  //check fo rmovie and should be public 
    if(!movie) return sendError(res,"Movie Not found!!",404);

    const isAlreadyReviewed= await Review.findOne({owner:userId,parentMovie:movie._id});
    if(isAlreadyReviewed) return sendError(res,"Invalid request, review is already given!",404);
...
exports.removeReview=async(req,res)=>{
    ...
    const review=await Review.findOne({owner:userId,_id:reviewId});
    if(!review) return sendError(res,"Invalid request,revire not found!");

    const movie=await Movie.findById(review.parentMovie).select('reviews');
    movie.reviews=movie.reviews.filter(rId=>rId.toString()!==reviewId); //remove the review from movie they should be same type to comapre
...
exports.getReviewsByMovie=async(req,res)=>{
    ...
    const reviews= movie.reviews.map(r=>{
        const {owner,content,rating,reviewID}=r
